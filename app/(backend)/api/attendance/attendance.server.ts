...
export const fetchAllAttendances = async () => {
    ...
    if (error) logger.error('supabase:database', 'attendance.server', `error fetching all attendances.`, error.message)
    return ({ data, error: error?.message });
};

export const fetchAttendance = async (id: number | undefined) => {
    if (!id) {
        logger.error('next:api', 'attendance.server', 'fetchAttendance id missing');
        return ({ data: null, error: 'fetchAttendance id missing' });
    }
    const supabase = createClient();
    const { data, error } = await supabase.from('attendance').select('*').eq('id', id).single();
    if (error) logger.error('supabase:database', 'attendance.server', `error fetching attendance ${id}...`, error.message)
    return ({ data, error: error?.message });
};

export const fetchAttendanceByRoomId = async (roomId: number | undefined) => {
    if (!roomId) {
        logger.error('next:api', 'attendance.server', 'fetchAttendanceByRoomId id missing');
    ...
    if (error) logger.error('supabase:database', 'attendance.server', `error fetching attendance with room ID ${roomId}...`, error.message);
    return ({ data, error: error?.message });
};
...
export const fetchAttendanceByUser = async (userId: string) => {
    if (!userId) {
        logger.error('next:api', 'attendance.server', 'fetchAttendanceByUser id missing');
    ...
    if (error) logger.error('supabase:database', 'attendance.server', `error fetching attendance with user ${userId.slice(0, 5)}...`, error.message)
    return ({ data, error: error?.message });
};
...
export const fetchNamesFromAttendance = async (userId: string) => {
    ...
    if (error || null) {
        logger.log('supabase:database', 'attendance.server', 'fetchNamesFromAttendance', `no names for user ${userId.slice(0, 5)}...`, error?.message);
        return ({ data: null, error: error ? error : 'No data found, null returned' });
    } else {
        logger.log('supabase:database', 'attendance.server', 'fetchNamesFromAttendance', `fetched names for user ${userId.slice(0, 5)}...`, data?.first_name, data?.last_name);
        return ({ data, error: null });
    }
};

export const fetchUserHasSignedAttendance = async (roomId: number, userId: string) => {
    logger.log('next:api', 'attendance.server', 'fetUserHasSignedAttendance', `roomId: ${roomId}, userId: ${userId}`);
    ...
    if (error) logger.error('supabase:database', 'attendance.server', 'fetchUserHasSignedAttendance', `error fetching attendance with room id ${roomId}...`, error.message);
    if (data && data?.length > 1) logger.warn('supabase:database', 'attendance.server', 'fetchUserHasSignedAttendance', `more than one attendance found with room id ${roomId}...`, data);
...
export async function countAttendances(roomId: string | number) {
    ...
    if (response.error) {
        logger.error('supabase:database', 'attendance.server', 'countAttendances', 'error counting attendances', response.error.message, 'discord')
        return 0
    }
