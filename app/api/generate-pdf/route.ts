...
export async function POST(req: NextRequest) {
	...
	logger.log("next:api", "api/generate-pdf", "Downloading pdf...");
	...
	const downloadImagesInBatches = async (urls: string[], batchSize: number) => {
		let results: Buffer[] = [];
		for (let i = 0; i < urls.length; i += batchSize) {
		const batch = urls.slice(i, i + batchSize);
		const batchResults = await Promise.all(batch.map(url => fetchImage(url)));
		results = [...results, ...batchResults];
		}
		return results;
	};

	const images = await downloadImagesInBatches(blobsUrls, 5);
